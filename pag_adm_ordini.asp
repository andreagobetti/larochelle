...
if oper="setingranaggio" then
	...
	Response.Cookies(questofile)("elencopredefinito")=request.form("elencopredefinito")
	Response.Cookies(questofile)("annopredefinito")=request.form("annopredefinito")
	oper="list"
end if
...
if operazione="da_stampare" then
	sql = "select * FROM ordini WHERE idord="&idord
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3

	rs("da_stampare")= not rs("da_stampare")
	if rs("da_stampare")=true then
		comunicazioni_precedenti= now()&" <b>"&session("nominativo")&":</b> impostato ordine da stampare"
	else
		comunicazioni_precedenti= now()&" <b>"&session("nominativo")&":</b> rimossa impostatazione ordine da stampare"
	end if
	rs("comunicazioni_precedenti")=rs("comunicazioni_precedenti")&"<hr />"& comunicazioni_precedenti
	rs.update
	rs.close
...
if request.form("operazione_ddt")="crea_bollettino" then
	sql="select * from ordini where idord="&idord
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3
	...
	set rs=Nothing
	call connclose()
	response.Redirect "bollettino.asp?idord="&idord
	'file_bollettino(idord)
end if

if operazione="peso_dimensione" then
	sql = "select * FROM ordini WHERE idord="&idord
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3
	rs("peso")=trim(request.form("peso_o"))
	rs("colli")=trim(request.form("colli_o"))
	rs.update
	rs.close
	set rs=Nothing
	oper="view"
  ...
  <div id="wrap">
    <div id="header"> <%=titolo_top%>
      ...
      <div id="barra_fissa">
        ...
        <div class="ui-widget-header ui-corner-all titolo_admin"><a href="<%=questofile%>">Ordini</a>
...
if oper="list" then
	...
	sql="select ordini.idord, ordini.tipo_documento, ordini.creato_da_admin, ordini.anno, ordini.nord, ordini.iduser, ordini.data, ordini.stato, utenti_intestazioni.nome, utenti_intestazioni.cognome, utenti_intestazioni.azienda, utenti_intestazioni.provincia, ordini.verde, utenti.email, utenti.cellulare, utenti.telefono, ordini.totale, ordini.indirizzo, ordini.citta, ordini_fatture.idfat, ordini.eliminato, ordini.idagente, ordini.primo, fatture.Nfat,fatture.pa, SUM(incassi.importo) AS SommaDiimporto, utenti.voto, utenti.tipologia, ordineddt.idord as idddt, ordineddt.nord as nddt FROM ((((ordini LEFT JOIN incassi ON ordini.idord = incassi.idord) LEFT JOIN utenti ON ordini.iduser = utenti.iduser) LEFT JOIN ordini_fatture ON ordini.idord = ordini_fatture.idord) LEFT JOIN fatture ON ordini_fatture.idfat = fatture.IDFat) left join utenti_intestazioni on ordini.idintestazione = utenti_intestazioni.id left join ddt on ordini.idord = ddt.sub_idord left join ordini ordineddt on ddt.idord = ordineddt.idord  GROUP BY ordini.idord "
		...
		where_fisso = " ordini.eliminato=0 and tipo_documento='ordine'"
		...
		if Request.Cookies(questofile)("annopredefinito")="" then
			where_anno=""
		elseif instr(anno,">")>0 then
		 ...
		 <%if carrello>0 then%>
          ...
          <tr>
            <td colspan="6"> Cerca
              ...
              <input type="button" value="Cerca articoli" style="FONT: 12px;" onclick="location.href='pag_adm_articoli_in_ordini.asp'">
              ...
              <input type="button" value="Nuovo..." id="pulsante_nuovo_ordine" style="display: none;">
				<input type="button" value="+Nuovo..." class="nuovodocumento" id="nuovo_ordine">
              <%end if %>
              </span></td>
          </tr>
          <tr>
...
%>
          <tr class="coprobox" <%=colore%>>
            ...
            <td style="border-top:1px solid;"><%=vedi_tipologia(objPagingRS("tipologia"))&primo_ordine(objPagingRS("primo"))%><b><%=denominazione(objPagingRS("nome"),objPagingRS("cognome"),objPagingRS("azienda"))%></b></td>
			...
			<%if objPagingRS("idddt")>0 then%> <a href="pag_adm_ddt.asp?idfat=<%=objPagingRS("idddt")%>">D <%=objPagingRS("nddt")%></a><%end if%>
			<%if objPagingRS("idfat")>0 then%> <a href="pag_adm_fatture.asp?idfat=<%=objPagingRS("idfat")%>">F <%=objPagingRS("nfat")&get_fattura_pa(objPagingRS("pa"),objPagingRS("fattura_ce")) %></a><%end if%></td>
          </tr>
...
'***********************************************************************************************************************************************
		...
		if nord<>"" then
			...
			sql1=sql1&"  where tipo_documento='ordine' and nord="&nord &"  and "&where_anno&" "
		...
		sql="select ordini.idord, ordini.nord from ordini inner join ordini_sostituzioni on ordini.idord = ordini_sostituzioni.idsostituzione  where  ordini_sostituzioni.idord="&idord
		set rs_sostituzione=conn.execute(sql)
		if not rs_sostituzione.eof then
			idsostituzione=rs_sostituzione("idord")
			nsostituzione=rs_sostituzione("nord")
			cancellabile=false
			esiste_sostituzione=true
		else
			esiste_sostituzione=false
		end if
		set rs_sostituzione = Nothing
 ...
 %>
 	...
 	<div id="multitabs" style="display:none;">
        <ul>
          <li><a href="#tabs-1" id="first_tab">Ordine <%=nord%></a></li>
		  ...
		  <%
			...
			sql="select ordini.idord, ordini.nord, ordini.data,ddt.causale, ddt.reso_tutto, ordini_fatture.idfat, ddt.da_fatturare FROM ddt inner join ordini on ddt.idord = ordini.idord LEFT JOIN ordini_fatture ON ordini.idord = ordini_fatture.idord where sub_idord="&idord
			set rs_ddt=conn.execute(sql)
			if not rs_ddt.eof then
				do while not rs_ddt.eof
					...
					if rs_ddt("da_fatturare")=1 and rs_ddt("reso_tutto") then
						colore="bgcolor='#ffe680'"	'Arancione chiaro reso tutto
						classe="arancione"
					elseif rs_ddt("da_fatturare")=0 then
						colore="bgcolor='#ffe680'"	'Arancione chiaro non fatturare
						classe="arancione"
					elseif not isnull(rs_ddt("idfat")) then
						colore="bgcolor='#99FF99'" 'Verdino saldata
						classe="verdino"
					else
						colore=""
				...
				%>
				<li><a href="pag_adm_ddt.asp?tab=1&idddt=<%=idddt%>" class="<%=classe%>"><strong>DDT <%=nddt%></strong> del <%=formatdatetime(data_ddt,2)%></a></li>
				<%
					rs_ddt.movenext
				loop
			end if
		  ...
		  <%end if%>
		  <%if esiste_sostituzione then %>
		  <li><a href="pag_adm_sostituzioni.asp?tab=1&idord=<%=idsostituzione%>"><strong>Sostituzione <%=nsostituzione%></strong></a></li>
		  <%end if%>
	...
	<div id="tabs-1" class="tab-panel">
        <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#E5E5E5" class="tabella1">
          <tr>
            <td colspan="2" class="ui-widget-header">Ordine
              ...
              <%end if %>
              <%if (not esiste_ddt) and (not esiste_fattura) and stato>=stato_min_ddt and stato<7 and ha_il_permesso("B1")  then%>
              ...
              <INPUT type="button" value="Modifica"  data-id="<%=idord%>" data-documento="ordine" class="pulsante_modifica_intestazione ui-button ui-widget ui-state-default ui-corner-all puls_">
              <button class="puls_giu" style="display:none;">Altro...</button>
              </span>
              <ul style="display:none;">
                <li><a href="pdf_ordine_preventivo.asp?tabella=ordini&per_cliente=si&idord=<%=idord%>">PDF per cliente</a></li>
                ...
                <%end if %>
                <%if esiste_sostituzione = false then %>
                <li ><a href="#" id="crea_sostituzione">Crea sostituzione</a></li>
                <%end if %>
          ...
          <tr >
            ...
            <td style="border-bottom:1px solid;"><b><%=ordine.campo("cognome")%>&nbsp;<%=ordine.campo("nome")%></b><%=vedi_tipologia(ordine.campo("tipologia"))&primo_ordine(ordine.campo("primo"))%><br>
        ...
        <%end if%>
        <%
		if ordine.campo("note_cliente")<>"" then%>
        <div class="ui-widget-header">Note cliente sull'ordine</div>
        <div class="div_testo notaarticolo"><%=ordine.campo("note_cliente")%></div>
	    <div class="div_spaziatore"></div>
        <%end if%>
...
<!-- incassi -->
	  ...
	  <%
	da_saldare=true
	  call tabella_incassi(idord,0,ordine.campo("totale"),true,true)
	  %>
		...
		call ordine.elenco_totali() %>
          ...
          <tr valign="middle" align="right">
            <td colspan="6" align="center" style="border-bottom: 1px solid gray; background: #E5E5E5; padding-top: 3px; padding-bottom:3px;">
              Articoli non modificabili perché ordine è saldato.
              </td>
          </tr>
          <%end if%>
        </table>
...
function Inseriscirisposta(){
        ...
        <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#E5E5E5" class="tabella1">
          ...
          <tr>
            <td width="16.6%" align="center" valign="bottom" ><input type="hidden" name="stato" value="" id="stato"/>
              <small><%=ordine.campo("data_stato1")%></small><br>
            ...
            <td width="16.6%" align="center" valign="bottom" class="stato_2"><small><%=ordine.campo("data_stato2")%></small><br>
            ...
            <td width="16.6%" align="center" valign="bottom" class="stato_3"><small><%=ordine.campo("data_stato3")%></small><br>
            ...
            <td width="16.6%" align="center" valign="bottom" class="stato_4"><small><%=ordine.campo("data_stato4")%></small><br>
              <button type="submit" name="" class="pulsanti ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only<%=disabilitato%>" value="<%=4%>" <%if stato=3 then%> style="background:#66c466;" <%end if%> onClick="stato.value=<%=4%>; return;" ><%=replace(stato_ordine(4),",","<br>")%></button></td>

            <td width="16.6%" align="center" valign="bottom" class="stato_5"><small><%=ordine.campo("data_stato5")%></small><br>
            ...
            <td width="16.6%"  align="center" valign="bottom" class="stato_6"><small><%=ordine.campo("data_stato6")%></small><br>
          ...
          <tr>
            <td width="16.6%" align="center" ><span class="piccolo">Modifica articoli
              </span></td>
            <td width="16.6%" align="center" ><span class="piccolo"><br>
              </span></td>
            <td width="16.6%" align="center" ><span class="piccolo">Abilita ddt e fattura</span></td>
                        <td width="16.6%" align="center" ><span class="piccolo"><br>
                          </span></td>

            <td width="16.6%" align="center" ><span class="piccolo">Invio mail<br>
        ...
        <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#E5E5E5" class="tabella1">
          ...
          <tr>
            ...
            <td width="20%" align="center" valign="bottom" ><%=ordine.campo("data_stato4")%><br>
              <button type="submit" name="" class="pulsanti ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only<%=disabilitato2%> stato2" value="<%=4%>" <%if stato=4 then%> style="background:#66c466;" <%end if%> onClick="stato.value=<%=4%>; return;" ><%=stato_ordine(4)%></button></td>
          ...
          <tr>
            ...
            <td width="20%" align="center" ><span class="piccolo">Invio mail <input type="checkbox" name="invia_mail" value="si" checked> <br>
              se ordine modificato allega elenco articoli</span></td>
...
sub ingranaggio()
elencopredefinito=Request.Cookies(questofile)("elencopredefinito")
anno=Request.Cookies(questofile)("annopredefinito")
 %>
...
<p>
 ...
 <p>
	 Visualizza ordini <input type="radio" name="annopredefinito" value="corrente" <%if anno="corrente" then response.write "checked" %>>Anno corrente  <input type="radio" name="annopredefinito" value="" <%if anno="" then response.write "checked" %>>Tutti
...
function primo_ordine(primo)
	primo_ordine=""
	'response.write tipologia
	if primo=1  then

	primo_ordine= "<div class='notification-bubble' style='background:red;'><span class='notification-number'>Primo ordine</span></div>"
	end if
end function
