<!--#include virtual="/setup.asp" -->
<!--#include virtual="/pag_adm_ordini_inc.asp" -->
Rivalorizza il campo taglia misura in ordini_dett_spettanze per nuovo rilievo<br>
<form action="<%=questofile%>" method="post">
	
<input type="submit" name="esegui" name="Esegui"/>
</form> 
<%
if request.form("esegui")<>"" then
	Server.ScriptTimeout=180
	sql="select * from dipendenti_misure where tipo_rilievo=1"
	
	
	set rs= conn.execute (sql)
	do while not rs.eof

		x=aggiorna_misure_in_ordini(rs("iddip"))
		response.write rs("iddip")&"<br>"
		rs.movenext
	loop
	rs.close
	set rs = nothing
	response.write "ESEGUITO"
end if
call connclose()

%>