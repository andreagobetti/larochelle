...
if request("send-contact")<>"" then
	...
	if txt_errore="" then
		...
		call add2log ("Accesso pagina manuals.asp",2)
	end if
...
<!--#include virtual="/config/header_inc.asp" -->
        		...
        		<div class="row">
        			<div class="col-md-12">
						<header class="content-title">
							<h1 class="title"><%=traduci("contus")%></h1>
							<p class="title-desc"><%=traduci("contustxt")%></p>
						</header>
						<%if session("passmanuali")<>true and session("idadmin")="" then%>
        				<div class="xs-margin"></div><!-- space -->
        					<div class="col-md-12 col-sm-12 col-xs-12">
        						<h2 class="sub-title"><%=traduci("txt11")%></h2>
        						<div class="row">
								<%if txt_errore<>"" then%>
									...
									<form name="contact-form" id="contact-form" method="post" action="<%=questofile%>">  
        								<div class="col-md-6 col-sm-12 col-xs-12">
											<div class="input-group">
												<span class="input-group-addon"><span class="input-icon input-icon-user"></span><span class="input-text"><%=traduci("psw")%>&#42;</span></span>
												<input type="password" name="passmanuali" id="passmanuali" required class="form-control input-lg" placeholder="<%=traduci("pswpl")%>">
											</div><!-- End .input-group -->
						...
						<%else %>
						<%if session("idadmin")<>"" then%>
							<p><b>Password per l'accesso:<%=pw_manuali%></b></p>
						<%end if%>
						<p><%=traduci("downtxt")%></p>
						<ul class="product-details-list">
						<%
						sql="select * from files where cosa=5"
	...
	$(function () {
	    ...
	    $('#fileupload').fileupload({
			url:'/sub_upload_new.asp?tipo_allegato=5&id_tipo_allegato=0&tipo_response=new',
	       dataType: 'json',
	        autoUpload: false,
