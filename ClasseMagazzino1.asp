...
Class ClasseMagazzino
	...
	Public sub scarica_ordine(idord)
		...
		on error goto 0
		'response.end
		call scorri_magazzino(rs_magazzino,2)
		response.write c_txt_log
		'call add2log(c_txt_log,0)
	end sub


    Private sub scorri_magazzino(rs_magazzino,operazione)
	    c_txt_log=c_txt_log&"scorri_magazzino(rs_magazzino, operazione:"&operazione&")<br>"
	    ...
	    if not rs_magazzino.eof then
		    do while not rs_magazzino.EOF
			    ...
			    c_txt_log=c_txt_log&"idmag:"&rs_magazzino("idmag")&"(quantita_magazzino:"&quantita_magazzino&", quantita_movimento:"&quantita_movimento&")"
			    ...
			    c_txt_log=c_txt_log&" dopo (quantita_magazzino:"&quantita_magazzino&", quantita_movimento:"&quantita_movimento&")<br>"
				...
				if rs_magazzino("db_acq")=1 then
					...
					call scorri_db_prodotti_vendita(rs_magazzino("idmag"),quantita_magazzino)
				end if
			...
			do while not rs_db.EOF
				...
				if quantita_massima>quantita_tmp or quantita_massima=0 then quantita_massima=quantita_tmp
					...
					if rs_db("db_ven")=1 then
						call scorri_db_prodotti_acquisto(rs_db("idmag_ven"),quantita_scarico)
					else
						call inserisci_movimento(rs_db("idmag"),quantita_scarico,operazione,c_idord,c_idfor)
					end if
				rs_db.movenext
			Loop	
		else
    ...
    Private sub scorri_db_prodotti_vendita(idmag_acq, quantita_magazzino)
	    c_txt_log=c_txt_log&"scorri_db_prodotti_vendita(idmag_acq:"&idmag_ven&", quantita:"&quantita&")<br>"
    	...
    	if not rs_db.eof then
			...
			if rs_db("db_acq")=1 then
				...
				call scorri_db_prodotti_vendita(rs_db("idpro_ven"),rs_db("idvara_ven"),rs_db("idvarb_ven"),quantita_magazzino)
			end if
    ...
    Private sub inserisci_movimento(idmag,quantita,operazione,idord,idfor)
	    c_txt_log=c_txt_log&"inserisci_movimento(idmag:"&idmag&", quantita:"&quantita&", operazione:"&operazione&", idord:"&idord&", idfor:"&idfor&")<br>"
	    dim sql, num
...
set rs_db_vend=conn.execute ("select distinta_base.*, magazzino.db_acq FROM magazzino INNER JOIN distinta_base ON (magazzino.idpro = distinta_base.idpro_ven) AND (magazzino.idvara = distinta_base.idvara_ven) AND (magazzino.idvarb = distinta_base.idvarb_ven) where idpro_acq="&idpro_acq&" and idvara_acq="&idvara_liv1&" and idvarb_acq="&idvarb_liv1)
	...
	sub movimento_magazzino(idpro,quantita,idvara,idvarb,causale,idord,idmag)
	'CAUSALE:
	'1 Rettifica giacienza
	'2 Scarido magazzino per ordine
	'3 Carico magazzino
			...
			if rs_magazzino("db_ven")=0 then
				...
				if  isnumeric(rs_magazzino("quantita_magazzino")) then 'PEZZA per il problema degli articoli con variante non specificata
					...
					if not isnull(quantita) then
						conn.execute ("update magazzino set quantita_magazzino="&metti_punto( quantita_magazzino) &" WHERE idmag="&rs_magazzino("idmag"))
						...
						quantita=replace(quantita,",",".")
						conn.execute "INSERT INTO magazzino_movimenti (data,idpro,quantita,idvara,idvarb,causale,iduser,idord) values (now(),"&rs_magazzino("idpro")&","&metti_punto( quantita) &","&rs_magazzino("idvara")&","&rs_magazzino("idvarb")&","&causale&","&session("iduser")&","&idord&");"	
					end if			
				end if
			else		'Distinta Base
