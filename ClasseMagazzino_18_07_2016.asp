...
Class ClasseMagazzino
	    ...
	    if not rs_magazzino.eof then
		    do while not rs_magazzino.EOF
		    	...
		    	select case operazione
		    		case 2	'Scarido magazzino per ordine
				...
				if operazione=2 then
					conn.execute ("update ordini_dett set quantita_scalato_magazzino=quantita_scalato_magazzino+"&metti_punto(quantita_movimento_originale)&" where iddett="&rs_magazzino("iddett"))
    ...
    Private sub aggiorna_magazzino(idmag,quantita)
		...
		sql="UPDATE magazzino SET magazzino.quantita_magazzino = "&metti_punto(quantita)&",data_aggiornamento=now() WHERE (((magazzino.idmag)="&idmag&"));"
		on error resume next
		conn.execute sql,num	
		if err.number<>0 then
			response.write c_txt_log&sql&"<br>"
    ...
    Private sub aggiorna_riordino(idmag,quantita)
		...
		sql="UPDATE magazzino SET magazzino.quantita_riordino = "&metti_punto(quantita)&" WHERE (((magazzino.idmag)="&idmag&"));"
		on error resume next
		conn.execute sql,num	
		if err.number<>0 then
			response.write c_txt_log&sql&"<br>"
